  ...
  useEffect(() => {
    ...
    if (featureGroupRef.current) {
      ...
      if (layers.length) {
        ...
        const latlngs = polyline.getLatLngs() as Array<L.LatLng>
    ...
    const calculateRoute = async () => {
      ...
      const coordinates = await getLineFromGraphHopper(
        ...
        {mode === 'drawing' &&
          points.map(([lng, lat], idx) => (
            ...
            >
              ...
              >
                {idx === 0
                  ? 'Origen (doble clic aquí para borrar)'
                  : idx === points.length - 1
                    ? 'Destino (doble clic aquí para borrar)'
                    : `Intermedio ${idx} (doble clic aquí para borrar)`}
              </Tooltip>
            </Marker>
          ))}
...
const BusLineGuide: React.FC<BusLineGuideProps> = ({
  points,
  mode,
  handleReset,
  maxPoints = 10,
}) => {
  const { newBusLine, busLineStep } = useBusLineContext()
    ...
    return (
      <div className="absolute top-2 left-2 z-[1100] bg-blue-50 border-l-4 border-blue-400 p-4 rounded-xl shadow-lg max-w-xs text-sm">
        <div className="mb-2 font-bold text-blue-800">
          ¿Cómo agregar paradas intermedias?
        </div>
        <ul className="list-disc ml-5 mb-2 space-y-1">
          ...
          <li>
            <b>Click sobre el recorrido:</b> se creará una nueva parada en ese
            punto.
          </li>
        </ul>
    ...
    <div className="absolute top-2 left-2 z-[1100] bg-gradient-to-br from-blue-50 to-white p-5 rounded-xl shadow-lg max-w-xs text-sm border border-blue-200">
      <div className="mb-3 flex items-center gap-2">
        <span className="font-bold text-base text-blue-800">
          Guía de creación de línea
        </span>
      </div>
      <ol className="mb-3 list-decimal list-inside space-y-2 pl-2">
        <li className="flex items-center gap-2">
          <span>
            <span className="font-semibold text-green-700">Origen:</span> Haz
            clic en una parada del mapa para establecer el punto de origen.
          </span>
        </li>
        <li className="flex items-center gap-2">
          <span>
            <span className="font-semibold text-blue-700">Intermedias:</span>{' '}
            Haz clic para agregar hasta <b>{maxPoints - 2}</b> puntos
            intermedios.
          </span>
        </li>
        <li className="flex items-center gap-2">
          <span>
            <span className="font-semibold text-red-700">Destino:</span> Haz
            clic en una parada del mapa para establecer el destino final.
          </span>
        </li>
        <li className="flex items-center gap-2">
          <span>
            <span className="font-semibold text-gray-700">Borrar:</span> Doble
            clic en cualquier marcador para eliminarlo.
          </span>
        </li>
      </ol>
      ...
      <div className="mb-2">
        ...
        {points.length === 0 && (
          <span className="text-gray-500">Selecciona el origen</span>
        )}
        {points.length === 1 && (
          <span className="text-blue-700">
            Selecciona el destino o puntos intermedios
          </span>
        )}
        {points.length > 1 && !finished && points.length < maxPoints && (
          <span className="text-blue-700">
            Puedes agregar intermedios o finalizar en el destino
          </span>
        )}
        ...
        {finished && (
          <span className="text-green-700 font-bold">
            ¡Destino marcado! Línea finalizada.
          </span>
        )}
      </div>
      {points.length > 0 && !finished && (
        <Button color="red" size="xs" onClick={handleReset} className="w-full">
          Limpiar puntos
        </Button>
      )}
    </div>
